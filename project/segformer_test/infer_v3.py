...
def main():
    ...
    def run_infer(mode):
        if (mode == 4):
            dataloader = [data_loaders[2],data_loaders[3],data_loaders[2],data_loaders[3] ]
        elif (mode == 3):
            dataloader = [data_loaders[2],data_loaders[0],data_loaders[3]]
        else:
        ...
        for dl in dataloader:
            ...
            for batch_ndx, sample in tqdm(enumerate(dl), total=len(dl)):
                ...
                if (mode == 4):
                    ...
                    #Selecting the max between the predictions that is rain or no rain
                    max_softmax_fog, _ = torch.max(gate_fog_output, dim=1)
                    max_softmax_night, _ = torch.max(gate_night_output, dim=1)
                    max_softmax_rain, _ = torch.max(gate_rain_output, dim=1)
                    max_softmax_snow, _ = torch.max(gate_snow_output, dim=1)


                    selected_gates = torch.stack([max_softmax_fog, max_softmax_night, max_softmax_rain,max_softmax_snow], dim=1)
                    # scaling the predictions 
                    weights = F.softmax(selected_gates, dim=1)

                    final_prediction = weights[:, 0].unsqueeze(1).unsqueeze(2).unsqueeze(3) * l1 + weights[:, 1].unsqueeze(1).unsqueeze(2).unsqueeze(3) * l2 +weights[:, 2].unsqueeze(1).unsqueeze(2).unsqueeze(3) * l3 +weights[:, 3].unsqueeze(1).unsqueeze(2).unsqueeze(3) * l4
                    ...
                    selected_gates = torch.stack([max_softmax_fog, max_softmax_night, max_softmax_rain], dim=1)
                    # scaling the predictions 
                    weights = F.softmax(selected_gates, dim=1)

                    final_prediction = weights[:, 0].unsqueeze(1).unsqueeze(2).unsqueeze(3) * l1 + weights[:, 1].unsqueeze(1).unsqueeze(2).unsqueeze(3) * l2 +weights[:, 2].unsqueeze(1).unsqueeze(2).unsqueeze(3) * l3
                    ...
                    selected_gates = torch.stack([max_softmax_fog, max_softmax_night], dim=1)
                    # scaling the predictions 
                    weights = F.softmax(selected_gates, dim=1)

                    final_prediction = weights[:, 0].unsqueeze(1).unsqueeze(2).unsqueeze(3) * l1 + weights[:, 1].unsqueeze(1).unsqueeze(2).unsqueeze(3) * l2 
    ...
    def load_gate(x):
        x = x.cuda()
        pretrained = '/BS/DApt/work/project/segformer_test/work_dirs/gate/NightvsFog_100.pth'
        gate = GatingNet()
        weights = torch.load(pretrained)
