...
def load_models():
    ...
    #RAIN
    config_path = '/BS/DApt/work/project/segformer_test/local_config/my_models/b5_rain_IDASS.py'
    cfg = Config.fromfile(config_path)
    ...
    #SNOW
    config_path = '/BS/DApt/work/project/segformer_test/local_config/my_models/b5_snow_IDASS.py'
    cfg = Config.fromfile(config_path)
...
class ReplayBuffer(Dataset):
    def __init__(self,dataloader1, dataloader2,dataloader3,dataloader4,num_samples=20):
        self.dl1 = dataloader1
        self.dl2 = dataloader2
        dataset1 = self.dl1.dataset
        dataset2 = self.dl2.dataset
        dataset3 = dataloader3.dataset
        dataset4 = dataloader4.dataset
        indices1 = random.sample(range(len(dataset1)), num_samples)
        indices2 = random.sample(range(len(dataset2)), num_samples)
        indices3 = random.sample(range(len(dataset3)), num_samples)
        indices4 = random.sample(range(len(dataset4)), num_samples)

        self.buffer = []
        for id1,id2,id3,id4 in zip(indices1,indices2,indices3,indices4):
            data1 = dataset1[id1]
            data2 = dataset2[id2]
            data3 = dataset3[id3]
            data4 = dataset4[id4]
...
class BalancedSampler(Sampler):
    def __init__(self, dataset, batch_size):
        ...
        self.rain_indices = [i for i, data in enumerate(dataset.buffer) if data['data_samples'].img_path.find("/rain/") != -1]
        self.snow_indices = [i for i, data in enumerate(dataset.buffer) if data['data_samples'].img_path.find("/snow/") != -1]
    ...
    def __iter__(self):
        ...
        random.shuffle(self.rain_indices)

        indices = []
        for _ in range(num_batches):
            fog_batch = random.sample(self.fog_indices, self.batch_size // 6)
            night_batch = random.sample(self.night_indices, self.batch_size // 2)
            rain_batch = random.sample(self.rain_indices, self.batch_size // 6)
            snow_batch = random.sample(self.snow_indices, self.batch_size // 6)
            batch = fog_batch + night_batch + rain_batch + snow_batch
...
def main():
    ...
    prev_path = '/BS/DApt/work/project/segformer_test/work_dirs/gate/snowvsraintvsrest_v4.pth'
    chk = torch.load(prev_path)
    gate_model.load_state_dict(chk)
    print('Incremental setting ON!')

    model_fog, model_night,model_rain,model_snow = load_models()
    data_loaders = load_dataloader()
    replay_buffer = ReplayBuffer(data_loaders[0],data_loaders[1],data_loaders[2],data_loaders[3])
    sampler = BalancedSampler(replay_buffer, batch_size=8)
    replay_loader = DataLoader(replay_buffer, batch_size=8, sampler = sampler, collate_fn=custom_collate)

    #import pdb;pdb.set_trace()

    criterion = nn.CrossEntropyLoss()
    optimizer = optim.AdamW(gate_model.parameters(), lr=1e-5)
    ...
    for epoch in range(50):
        ...
        for batch_ndx, sample in enumerate(replay_loader):
            ...
            for i in range(img.size(0)):
                ...
                if img_metas[i]['img_path'].find("/rain/") != -1:
                    labels[i] = 1
                elif img_metas[i]['img_path'].find("/night/") != -1:
                    labels[i] = 0
                elif img_metas[i]['img_path'].find("/fog/") != -1:
                    labels[i] = 1
            ...
            if batch_ndx % 10 == 0:  
                ...
                print(f'[epoch {epoch + 1}, accuracy {accuracy:4f}] loss: {run_loss / 5:.6f}')
                run_loss = 0.0

    save_path = f'/BS/DApt/work/project/segformer_test/work_dirs/gate/nightwvsrest_v3.pth'
