...
def load_models():
    ...
    return backbone_fog,backbone_night,backbone_rain
...
class ReplayBuffer(Dataset):
    def __init__(self,dataloader1, dataloader2,dataloader3,num_samples=20):
        ...
        self.buffer = []
        for id1,id2,id3 in zip(indices1,indices2,indices3):
            data1 = dataset1[id1]
            data2 = dataset2[id2]
            data3 = dataset3[id3]
...
class BalancedSampler(Sampler):
    def __init__(self, dataset, batch_size):
        ...
        self.night_indices = [i for i, data in enumerate(dataset.buffer) if data['data_samples'].img_path.find("/rain/") != -1]
        self.rain_indices = [i for i, data in enumerate(dataset.buffer) if data['data_samples'].img_path.find("/snow/") != -1]
    ...
    def __iter__(self):
        ...
        for _ in range(num_batches):
            fog_batch = random.sample(self.fog_indices, self.batch_size // 4)
            night_batch = random.sample(self.night_indices, self.batch_size // 2)
            rain_batch = random.sample(self.rain_indices, self.batch_size // 4)
            batch = fog_batch + night_batch + rain_batch
...
def main():
    ...
    prev_path = '/BS/DApt/work/project/segformer_test/work_dirs/gate/RainvsFog_v1_vpt.pth'
    chk = torch.load(prev_path)
    gate_model.load_state_dict(chk)
    print('Incremental setting ON!')


    model_fog, model_night,model_rain = load_models()
    data_loaders = load_dataloader()
    replay_buffer = ReplayBuffer(data_loaders[0],data_loaders[1],data_loaders[2])
    ...
    save_path = f'/BS/DApt/work/project/segformer_test/work_dirs/gate/snowvsraintvsrest_v4.pth'
